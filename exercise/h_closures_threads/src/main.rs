...
fn main() {
    ...
    let handle_a = thread::spawn(move || {
        const DELAY_TO_MAKE_THREAD_B_FIRST: u64 = 400;
        pause_ms(DELAY_TO_MAKE_THREAD_B_FIRST + 0);
        tx2.send("Thread A: 1").unwrap();
        pause_ms(DELAY_TO_MAKE_THREAD_B_FIRST + 200);
    ...
    let (tx, rx1): (Sender<i32>, Receiver<i32>) = channel::unbounded();
    let rx2 = rx1.clone();
    let handle_first = thread::spawn(move || {
        for msg in rx1 {
            println!("Thread FIRST received: {}", msg);
        }
    });
    let handle_second = thread::spawn(move || {
        for msg in rx2 {
            println!("Thread SECOND received: {}", msg);
        }
    });
    pause_ms(1000);
    for i in 0..50 {
